...
@objc protocol BookDetailTableViewDelegate {
  ...
  func moreBooksTapped(forLane: TPPCatalogLane)
  func viewIssuesTapped()
}


final class TPPBookDetailTableView: UITableView {
...
@objcMembers final class TPPBookDetailTableViewDelegate: NSObject, UITableViewDataSource, UITableViewDelegate {
  ...
  weak var laneCellDelegate: TPPCatalogLaneCellDelegate?
  ...
  var catalogLaneCells = [TPPCatalogLaneCell]()
  var catalogLanes = [TPPCatalogLane]()
  
  init (_ tableView: UITableView, book: TPPBook) {
...
//    }
  ...
  private func createLaneCells(_ groupedFeed: TPPCatalogGroupedFeed?) {
  ...
  func checkAndRemoveRedundantTitles(_ lane: TPPCatalogLane, _ index: inout UInt) {
    if (lane.books.count == 1) {
      if (lane.books[0] as! TPPBook).title == self.book.title {
  ...
  @objc func configureViewIssuesCell() {
    ...
    // Visibility logic
    if TPPProblemDocumentCacheManager.shared.getLastCachedDoc(self.book.identifier) == nil {
